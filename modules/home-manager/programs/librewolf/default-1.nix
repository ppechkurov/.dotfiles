{ pkgs, ... }: {
  ...
  programs.librewolf.package =
    let policiesJson = pkgs.callPackage ./policies.nix { };
    in pkgs.librewolf.override {
      # extraPoliciesFiles = [ policiesJson ];
      extraPoliciesFiles = pkgs.librewolf.unwrapped.extraPoliciesFiles
        ++ [ policiesJson ];
    };
  ...
  programs.librewolf.settings = {
    ...
    "Preferences.layout.spellcheckDefault.Value" = 0;
    "Preferences.layout.spellcheckDefault.Status" = "locked";
    ...
    # "browser.safebrowsing.downloads.remote.url" = false; # this throws an error
